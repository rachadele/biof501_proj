...
for query_name, query in queries.items():
    ...
    for ref_name,ref in refs.items():
        all_probs = classify_cells(ref=ref, query=query, ref_keys=ref_keys)
        probs[query_name][ref_name] = all_probs
        queries[query_name],results[query_name][ref_name] = evaluate_classifier(probabilities=all_probs, query=query, ref_keys=ref_keys)
        
        new_query_name = query_name.replace(" ", "_").replace("/", "_")
        new_ref_name = ref_name.replace(" ", "_").replace("/", "_")
        plot_roc_curves(metrics=results[query_name][ref_name], 
                        title=f"{query_name} vs {ref_name}",
                        save_path=os.path.join(projPath,"results",f"{new_query_name}_{new_ref_name}_roc_results.png"))

all_f1_scores=combine_f1_scores(results) 

        #i need to combine these classification dicts into a pandas data frame
# %%
